...
|:--:|:----|:---------|
| ![](figures/fig-sym-entity.svg){width="86" fig-alt="Rectangle"} | External entity | People or systems outside your control: The customer, the payment provider, the tax authority |
| ![](figures/fig-sym-process.svg){width="52" fig-alt="Circle"} | Process | Changes data; at least one flow in and one out; the whole system is process 0 |
| ![](figures/fig-sym-store.svg){width="86" fig-alt="Open double lines"} | Data store | Data at rest - database, file, backup; read or written only by a process |
| ![](figures/fig-sym-flow.svg){width="86" fig-alt="Labelled arrow"} | Data flow | Data in motion, labelled by content, not technology |
| ![](figures/fig-sym-tb.svg){width="86" fig-alt="Dashed line"} | Trust boundary | Where the level of trust changes; every crossing arrow is an entry point |
...
::: {#tbl-strideelement}
  **Element**          **S**       **T**            **R**            **I**       **D**       **E**    **FastGoods element →             candidate threat**
  ----------------- ----------- ----------- --------------------- ----------- ----------- ----------- --------------------------------------------------------------------------------------------
  External entity    ●                            ●                                                   Payment provider: A forged "payment OK" callback (S); a customer denies an order later (R)
  Process            ●           ●                ●                ●           ●           ●          Checkout: Flooded offline (D =   R2); a price altered in transit (T)
  Data flow                      ●                                 ●           ●                      Credentials over TB1: Replayed from a leak (feeds R1); read in transit without TLS (I)
  Data store                     ●           ●†                    ●           ●                      Customer database D1: Bulk read (I =   R3); records edited with no audit log (R)

  : STRIDE per element (Shostack 2014, Table 3.1) applied to FastGoods.
  †           Only for a store that holds logs or evidence. Two of the
...
source\] →             \[event\] exploiting \[vulnerability\] →
\[asset\] →             \[consequence\]. A statement in this form is assessable;
a fragment is not.
:::
...
::: {#exr-08-stride-interactive}
## STRIDE and the properties, interactive

Match each STRIDE threat to the property it attacks.
...
<script type="application/json">
{
 ...
 "explain": "STRIDE is a mnemonic over the properties: Each letter is the attack on one of them.",
 "ref": "sec-08-mapping-the-system-the-data-flow",
 "reflabel": "Mapping the system"
}
</script>
</div>
```
...
::: {#exr-08-dfd-interactive}
## DFD elements, interactive

Assign each part of the FastGoods Level 1 diagram to its DFD element type.
...
<script type="application/json">
{
 ...
 "chips": [
  ...
  {
   "t": "D1, the customer database",
   "b": "store"
  },
  {
   "t": "Credentials crossing TB1",
   "b": "flow"
  },
  {
   "t": "The payment provider",
   "b": "ent"
  },
  {
   "t": "The back office",
 ...
 "explain": "Four symbols carry the whole notation: Boxes outside your control, circles that change data, open lines where it rests, arrows where it moves.",
 "ref": "sec-08-mapping-the-system-the-data-flow",
 "reflabel": "Mapping the system"
}
</script>
...
::: {#exr-08-statement-interactive}
## Anatomy of a risk statement, interactive

Arrange the parts of a well-formed risk statement.
...
<script type="application/json">
{
 ...
 "explain": "A statement in this five-part form is assessable: Each part can be challenged, and each maps to a register column.",
 "ref": "sec-08-formulating-the-risks",
 "reflabel": "Formulating the risks"
}
</script>
</div>
```
...
::: {#exr-08-dfdrules-interactive}
## Rules of an honest diagram, interactive

Select the drawing rules this chapter sets for a DFD.
...
<script type="application/json">
{
 "options": [
  {
   "t": "Every process has at least one flow in and one out",
   "ok": true
  },
  {
   "t": "A store is read or written only through a process",
   "ok": true
  },
  {
   "t": "Flows are labelled with the data, not the technology",
   "ok": true
  },
  {
   "t": "The diagram shows where data moves, not in what order",
   "ok": true
  },
  {
   "t": "Every arrow must name its protocol",
   "ok": false
  },
  {
   "t": "Level 2 is always required",
   "ok": false
  }
 ],
 "explain": "The rules keep the drawing honest - and a rule violation usually marks a forgotten process, which is where the missing control sits.",
 "ref": "sec-08-mapping-the-system-the-data-flow",
 "reflabel": "Mapping the system"
}
</script>
...
::: {#exr-08-sources-interactive}
## Four sources of risk, interactive

Match each activity to the source of risk it works from.
...
<script type="application/json">
{
 "buckets": [
  {
   "id": "proc",
   "label": "A business process, walked"
  },
  {
   "id": "sys",
   "label": "A system or IT process"
  },
  {
   "id": "ctrl",
   "label": "A control read backwards"
  },
  {
   "id": "audit",
 ...
 "chips": [
  {
   "t": "Stepping through the order process stage by stage",
   "b": "proc"
  },
  {
   "t": "Reviewing the backup system's configuration",
   "b": "sys"
  },
  {
   "t": "Testing whether the access review actually happens",
   "b": "ctrl"
  },
  {
   "t": "Reading last year's audit report as a risk list",
   "b": "audit"
  }
 ],
 "explain": "A risk that cannot name its source has been produced, not found - the four sources are where real risks come from.",
 "ref": "sec-08-formulating-the-risks",
 "reflabel": "Formulating the risks"
}
</script>
...
::: {#exr-08-history-interactive}
## The diagram's lineage, interactive

Order the milestones of the data-flow diagram's security career.
...
<script type="application/json">
{
 "steps": [
  ...
  "STRIDE (1999)",
  "Trust boundaries in Microsoft's SDL (2004)",
  "LINDDUN privacy threats (2010)",
  "The Threat Modeling Manifesto (2020)"
 ],
 "explain": "The diagram is older than its security career: Drawn since 1978, threat-modelled since STRIDE, boundary-marked since the SDL.",
 "ref": "sec-08-mapping-the-system-the-data-flow",
 "reflabel": "Mapping the system"
}
</script>
